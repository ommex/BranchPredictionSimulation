...
def plotter_runner(folder):
    ...
    print(
        "Geben sie einen reinen Dateinamen (ohne Dateiendung!) an wie die Tabelle heißen soll"
    )
    ...
    if predictor_i == 0:
        ...
        if local_predictor_mode == "0":
            print(
                "Geben sie ein wie viele Bits von links einer Adresse maximal entfernt werden sollen. Für 10 Bit die übrig bleiben wäre es z.B. 32-10=22 \nFor example: 111100 with bitcrop 2 -> 1100"
            )
            ...
            local_iterators.bit_crop_size_iterator(
                end_bit_crop,
                folder,
                save_file_name,
            ...
            local_iterators.pht_size_iterator(
                pht_size,
                folder,
                save_file_name,
                checkpoint="checkpoint.json",
        ...
        global_iterators.history_size_iterator(
            history_end_size,
            folder,
            save_file_name,
            pht_size=pht_size,
        ...
        hybrid_iterators.gshare_size_iterator(
            history_end_size,
            folder,
            save_file_name,
            pht_size=pht_size,
        ...
        if tournament_mode == 1:
            ...
            hybrid_iterators.tournament_global_history_iterator(
            ...
            hybrid_iterators.tournament_local_history_iterator(
            ...
            hybrid_iterators.tournament_bit_crop_iterator(
                global_history_len,
                local_history_len,
                pht_size,
                bit_crop,
...
def single_value_runner(folder):
    ...
    if predictor_i in [0, 3]:
        ...
        if predictor_i == 0:
            print(
                "Running local_predictor for PHT-size: "
                + str(pht_size)
                + " BitCrop: "
                + str(bit_crop)
            )
            predictor_test.test_local_predictor(
                folder, "checkpoint.json", pht_size, bit_crop
            )
        else:
            print("Geben sie die Länge in Bit der Globalen Historie ein.\nFür 4 ENTER")
            global_history_len = input()
            ...
            print(
                "Running tournament_predictor für globale_history_len: "
                + str(global_history_len)
                + " local_history_len: "
                + str(local_history_len)
                + " Pht-size: "
                + str(pht_size)
                + " BitCrop: "
                + str(bit_crop)
            )
            predictor_test.test_tournament_predictor(
                folder,
                "checkpoint.json",
                global_history_len,
                local_history_len,
        ...
        if predictor_i == 1:
            print(
                "Running two_level_global predictor mit Pht-size: "
                + str(pht_size)
                + " history_len: "
                + str(global_history_len)
            )
            predictor_test.test_ght_predictor(
                folder, "checkpoint.json", pht_size, global_history_len
            )
        else:
            print(
                "Running gshare predictor mit pht-size: "
                + str(pht_size)
                + " history_len: "
                + str(global_history_len)
            )
            predictor_test.test_gshare_predictor(
                folder, "checkpoint.json", pht_size, global_history_len
            )
...
if __name__ == "__main__":
    ...
    print(
        "Um bei wiederholter Ausführung schneller zu sein werden die .txt trace files einmal eingelesen und zu einem Checkpoint verarbeitet."
    )
    print(
        "Bitte geben Sie jetzt den relativen Pfad zum Ordner an indem diese gespeichert sind. Für 'trace' ENTER"
    )
    folder = input()
    if folder == "":
        folder = "trace"
