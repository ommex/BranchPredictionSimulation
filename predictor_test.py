...
def test_tournament_predictor(folder_name, checkpoint, global_history_len, local_history_len, pht_len, bit_crop):
    output = []
    traces = open_traces(folder_name, checkpoint)
    ...
    for fname in traces.keys():
        ...
        global_predictor = predictors.n_bit_tournament(local_history_len, global_history_len, pht_len = pht_len, bit_crop = bit_crop)
        simulation_results = global_predictor.check_traces(file)
        output.append(simulation_results)
        print(simulation_results)
